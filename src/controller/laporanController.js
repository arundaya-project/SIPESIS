...
class LaporanController {
    ...
    async createLaporan(req, res) {
        try {
            ...
            const [siswa] = await pool.query('SELECT * FROM data_siswa WHERE id = ?', [siswa_id]);
            if (siswa.length === 0) {
                return res.status(404).json({ message: "Student not found" });
            }

            const [result] = await pool.query(
                'INSERT INTO laporan (siswa_id, title, description, jenis_pelanggaran, tanggal) VALUES (?, ?, ?, ?, ?)',
                [siswa_id, title, description, jenis_pelanggaran, date]
            );
